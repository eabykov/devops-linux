...
<details>
  ...
  6. `df -m` (или можно -h) и `iostat -dx DISK_NAME` где DISK_NAME взято из столбца `Filesystem` первой команды
...
1. Создать скрипт который:
   ...
   4. Установить docker - запускает контейнеры (в которых упакованы приложения со всем что им нужно для запуска)
