...
1. Создать скрипт который:
   ...
   3. Устанавливает (только если еще не установлены)
      - wget - скачивает файлы из интернета, можно проверить доступность сайта
      - curl - выполнять http запросы
      - helm - устанавливает приложения в Kubenretes со всеми настройками
      - docker - запускает контейнеры (в которых упакованы приложения со всем что им нужно для запуска)
